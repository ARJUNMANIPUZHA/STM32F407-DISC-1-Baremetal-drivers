...
typedef struct
{
	__vo uint32_t IMR;     /*!< Interrupt mask register
	                             Enables or disables interrupt request from EXTI lines.
	                             0 = masked, 1 = interrupt enabled                  */

	__vo uint32_t EMR;     /*!< Event mask register
	                             Enables or disables event generation from EXTI lines
	                             (used for event system instead of IRQ)             */

	__vo uint32_t RTSR;    /*!< Rising trigger selection register
	                             Selects rising edge trigger for EXTI lines         */

	__vo uint32_t FTSR;    /*!< Falling trigger selection register
	                             Selects falling edge trigger for EXTI lines        */

	__vo uint32_t SWIER;   /*!< Software interrupt event register
	                             Writing 1 generates software interrupt on EXTI line*/

	__vo uint32_t PR;      /*!< Pending register
	                             Indicates which EXTI line triggered.
...
typedef struct
{
	__vo uint32_t MEMRMP;       /*!< Memory remap register
	                                 Controls memory mapping at address 0x00000000
	                                 (Flash / System Memory / SRAM remap)            Address offset: 0x00 */

	__vo uint32_t PMC;          /*!< Peripheral mode configuration register
	                                 Controls Ethernet PHY interface selection
	                                 (MII/RMII configuration)                         Address offset: 0x04 */

	__vo uint32_t EXTICR[4];    /*!< External interrupt configuration registers
	                                 Select GPIO port source for EXTI lines
	                                 EXTI0–EXTI15 mapping to GPIOA–GPIOI             Address offset: 0x08-0x14 */

	uint32_t      RESERVED1[2]; /*!< Reserved                                       Reserved, 0x18-0x1C */

	__vo uint32_t CMPCR;        /*!< Compensation cell control register
	                                 Enables I/O compensation cell for high-speed
	                                 GPIO operation                                   Address offset: 0x20 */

	uint32_t      RESERVED2[2]; /*!< Reserved                                       Reserved, 0x24-0x28 */

	__vo uint32_t CFGR;         /*!< Configuration register
	                                 Controls I2C Fast-mode Plus and other
	                                 system-level configuration bits                  Address offset: 0x2C */
} SYSCFG_RegDef_t;
...
typedef struct
{
	__vo uint32_t CR1;      /*!< SPI control register 1
	                             Configures SPI mode, clock polarity/phase,
	                             master/slave selection, baud rate, data frame
	                             format, software slave management, enable bit     Address offset: 0x00 */

	__vo uint32_t CR2;      /*!< SPI control register 2
	                             Interrupt enables, DMA control, SS output enable,
	                             frame format and error interrupt control           Address offset: 0x04 */

	__vo uint32_t SR;       /*!< SPI status register
	                             Indicates TXE, RXNE, BUSY, CRCERR, MODF,
	                             OVR and other status flags                         Address offset: 0x08 */

	__vo uint32_t DR;       /*!< SPI data register
	                             Used for transmit and receive data                 Address offset: 0x0C */

	__vo uint32_t CRCPR;    /*!< SPI CRC polynomial register
	                             Defines polynomial value used for CRC calculation  Address offset: 0x10 */

	__vo uint32_t RXCRCR;   /*!< SPI RX CRC register
	                             Stores calculated CRC value for received data      Address offset: 0x14 */

	__vo uint32_t TXCRCR;   /*!< SPI TX CRC register
	                             Stores calculated CRC value for transmitted data   Address offset: 0x18 */

	__vo uint32_t I2SCFGR;  /*!< I2S configuration register
	                             Selects I2S mode, audio standard, data length,
	                             channel configuration and master/slave mode        Address offset: 0x1C */

	__vo uint32_t I2SPR;    /*!< I2S prescaler register
	                             Configures I2S clock prescaler for audio sampling
	                             frequency generation                               Address offset: 0x20 */
} SPI_RegDef_t;
...
typedef struct
{
  __vo uint32_t CR1;     /*!< I2C control register 1
                               Peripheral enable, ACK control, START/STOP generation,
                               software reset, clock stretching control              Address offset: 0x00 */

  __vo uint32_t CR2;     /*!< I2C control register 2
                               Peripheral clock frequency configuration,
                               interrupt enable bits (ITEVTEN, ITBUFEN, ITERREN)     Address offset: 0x04 */

  __vo uint32_t OAR1;    /*!< Own address register 1
                               Primary device address in slave mode,
                               addressing mode selection (7-bit/10-bit)              Address offset: 0x08 */

  __vo uint32_t OAR2;    /*!< Own address register 2
                               Secondary slave address configuration                  Address offset: 0x0C */

  __vo uint32_t DR;      /*!< Data register
                               Used to transmit and receive data bytes                Address offset: 0x10 */

  __vo uint32_t SR1;     /*!< Status register 1
                               Contains event/error flags like SB, ADDR, BTF,
                               RXNE, TXE, STOPF, AF, OVR                              Address offset: 0x14 */

  __vo uint32_t SR2;     /*!< Status register 2
                               Bus status flags like BUSY, MSL, TRA,
                               and dual address indication                            Address offset: 0x18 */

  __vo uint32_t CCR;     /*!< Clock control register
                               Configures SCL clock speed (Standard/Fast mode)
                               and duty cycle settings                                Address offset: 0x1C */

  __vo uint32_t TRISE;   /*!< TRISE register
                               Maximum rise time configuration for SCL line           Address offset: 0x20 */

  __vo uint32_t FLTR;    /*!< Filter register
                               Configures analog and digital noise filters            Address offset: 0x24 */
} I2C_RegDef_t;
